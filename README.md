...
ExportAsText exports all text files as UTF-8 wihtout a BOM.  The UTF-8 standard
...
Files not ending in .csv are imported using the standard OpenText() function
with the appropriate arguments.  Files ending in .csv must be opened with
a custom function, due to OpenText() ignoring most, if not all, of the
various options you pass it when a file ends in .csv.  The custom function
uses QueryTable() to load the data in via a database query, which actually
honors all the options that you set for it.  Thank you EEM of Stackoverflow!
